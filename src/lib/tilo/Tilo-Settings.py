...
class TiloSettings:
	...
	def __init__(self):
		...
		self.window.set_icon(GdkPixbuf.Pixbuf.new_from_file(Globals.Applogo))
		...
		self.label_tab1 = Gtk.Label(label=_('Themes'))
		self.label_tab2 = Gtk.Label(label=_('Preferences'))
		self.label_tab3 = Gtk.Label(label=_('About'))
		self.label_tab4 = Gtk.Label(label=_('Commands'))
		...
		if self.window.size_request()[1] +50 > Gdk.Screen.height():
	...
	def add_preferences_tab(self):
		...
		self.label11 = Gtk.Label(label=_('Icon Size in Program List'))
		self.label11.set_justify(Gtk.Justification.LEFT)
		self.label12 = Gtk.Label(label=_('Number of Items in Recent Items List'))
		...
		self.label14 = Gtk.Label(label=_('Tab hover effect'))
		...
		self.label13 = Gtk.Label(label=_('Program list type'))
	...
	def add_commands_tab(self):
		...
		self.label1 = Gtk.Label(label=_('Search'))
		...
		self.label3 = Gtk.Label(label=_('Network Config'))
		...
		self.label4 = Gtk.Label(label=_('Control Panel'))
		...
		self.label5 = Gtk.Label(label=_('Package Manager'))
		...
		self.label6 = Gtk.Label(label=_('Help'))
		...
		self.label7 = Gtk.Label(label=_('Shutdown Dialog'))
		...
		self.label8 = Gtk.Label(label=_('Lock Screen'))
		...
		self.label26 = Gtk.Label(label=_("Logout"))
		self.label26.set_justify(Gtk.Justification.LEFT)
		...
		self.label9 = Gtk.Label(label=_('Logout Dialog'))
		...
		self.label2 = Gtk.Label(label=_('Restart'))
		...
		self.label20 = Gtk.Label(label=_('Suspend'))
		self.label20.set_justify(Gtk.Justification.LEFT)
		...
		self.label21 = Gtk.Label(label=_('Hibernate'))
		self.label21.set_justify(Gtk.Justification.LEFT)
		...
		self.label22 = Gtk.Label(label=_('Run'))
		self.label22.set_justify(Gtk.Justification.LEFT)
		...
		self.label23 = Gtk.Label(label=_('Shutdown'))
		self.label23.set_justify(Gtk.Justification.LEFT)
		...
		self.label24 = Gtk.Label(label=_('User about'))
		self.label24.set_justify(Gtk.Justification.LEFT)
		...
		self.label25 = Gtk.Label(label=_("Open as Administrator"))
		self.label25.set_justify(Gtk.Justification.LEFT)
		...
		self.label27 = Gtk.Label(label=_("Menu Editor"))
		self.label27.set_justify(Gtk.Justification.LEFT)
	...
	def add_about_tab(self):
		...
		self.Applogo = GdkPixbuf.Pixbuf.new_from_file_at_size(Globals.Applogo,80,80)
		self.image_logo.set_from_pixbuf(self.Applogo)
		self.label_app = Gtk.Label(label="Tilo %s" % Globals.version)
		self.label_credits = Gtk.Label(label="Consolidated menu for the MATE desktop.\nBy Helder Fraga aka Whise <helderfraga@gmail.com>\n Please Donate")
		self.font_desc = Pango.FontDescription('sans bold 14')
		self.label_app.modify_font(self.font_desc)
		self.label_credits.set_justify(Gtk.Justification.CENTER)
		...
		self.button9 = Gtk.Button(_('Tilo Themes Specifications'))
	...
	def button_clicked (self, widget, id):
		...
		if id == 'Donate':
			...
			os.system('xdg-open http://launchpad.net/tilo/trunk/2.3/+download/TiloThemeSpec.pdf &')
		elif id == 'Bug':
			os.system('xdg-open https://bugs.launchpad.net/tilo &')
		elif id == 'Translate':
			os.system('xdg-open https://translations.launchpad.net/tilo/trunk &')
		elif id == 'Homepage':
			os.system('xdg-open https://launchpad.net/tilo &')

		elif id == 'Credits':
			try:
				import mate.ui
				self.Applogo = GdkPixbuf.Pixbuf.new_from_file_at_size(Globals.Applogo,80,80)
				about = mate.ui.About(Globals.name,Globals.version,"GPL","Consolidated menu for the MATE desktop.",["Helder Fraga aka Whise <helderfraga@gmail.com>" , "https://launchpad.net/~tilo-team"],["https://launchpad.net/~tilo-team"],"https://translations.launchpad.net/tilo/trunk/+pots/tilo", self.Applogo)
				about.show()
			except:pass
	...
	def reload_themes(self,id):
		...
		if id == 'Menu':
			...
			else:self.label_credits_sound = Gtk.Label(label='')
	...
	def redraw_image(self,widget,id):
		...
		if id == 'Menu':
			try:
				Pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size("%sMenu/%s/themepreview.png" % (Globals.ThemeDirectory, widget.get_active_text()),200,230)
				#self.image_menu.set_
			except:
				Pixbuf = GdkPixbuf.Pixbuf.new_from_file("%stheme.png" % Globals.GraphicsDirectory)
			...
			try:
				Pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size("%sButton/%s/themepreview.png" % (Globals.ThemeDirectory, widget.get_active_text()),64,64)
			except:
				Pixbuf = GdkPixbuf.Pixbuf.new_from_file("%stheme.png" % Globals.GraphicsDirectory)
			...
			try:
				Pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size("%sIcon/%s/themepreview.png" % (Globals.ThemeDirectory, widget.get_active_text()),64,64)
			except:
				Pixbuf = GdkPixbuf.Pixbuf.new_from_file("%stheme.png" % Globals.GraphicsDirectory)
	...
	def show_install_dialog (self):
		...
		if resp == Gtk.ResponseType.OK:
			...
			self.window.window.set_cursor(Gdk.Cursor.new(Gdk.CursorType.LEFT_PTR))
	...
	def install(self,filename):
		...
		if type == 'Icon':
			...
			if x != 0:
				utils.show_message(_('This theme was created for a previous version of Tilo.\nThe theme was automaticly updated in order to work properly.'))
